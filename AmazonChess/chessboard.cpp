...
// TTEntry 结构体
struct TTEntry {
    uint64_t key;     // ZobristKey，用于校验是否发生哈希冲突
    int score;        // 评估分数
    int depth;        // 搜索该分数时的剩余深度
    int flag;         // 标志位： EXACT (精确值), ALPHA_CUT (上限), BETA_CUT (下限)
    ChessBoard::AIMove bestMove;  // 在该局面下搜索到的最佳走法 (用于走法排序)
};

// 标志位常量
const int TT_FLAG_EXACT = 0;
const int TT_FLAG_ALPHA = 1; // 实际值可能大于等于 score (下限)
const int TT_FLAG_BETA = 2;  // 实际值可能小于等于 score (上限)

// 定义置换表（大小应为质数，例如 2^20 或 1M 条目）
uint64_t ZobristKeys[10][10][4];
const int TT_SIZE = 1048576; // 2^20
...
int ChessBoard::alphaBeta(int depth, int alpha, int beta, int player) {
    ...
    uint64_t currentKey = currentZobristKey; // 假设 currentZobristKey 随时更新
    int ttIndex = currentKey % TT_SIZE;
    TTEntry& entry = TranspositionTable[ttIndex];
    int originalAlpha = alpha;
    int originalBeta = beta;
    ChessBoard::AIMove ttBestMove;

    // 2. 查找（Lookup）
    if (entry.key == currentKey) { // 校验Key
        if (entry.depth >= depth) { // 深度足够
    ...
    const int TT_SCORE_BONUS = 1000000;

    // 1. 计算每个走法的排序分数
    for (auto& m : moves) {
        // 使用历史表分数作为基础分数
        // 历史表权重可以基于深度平方累积，所以这里权重可以低一些
        m.score = history_table[m.qx][m.qy][m.mx][m.my];

        // 2. 赋予 TT 走法最高分数
        if (m == ttBestMove) {
            m.score += TT_SCORE_BONUS; // 确保它排在第一位
        }

        // 3. (可选) 增加杀手走法（Killer Moves）逻辑
        // 如果 m 是 Killer Move，增加次高分数（例如 500000）
        // (如果您想实现 Killer Moves，则需要在这里添加逻辑)
    }

    // 3. 执行排序
    std::sort(moves.begin(), moves.end(), compareAIMoves);

    int opponent = 3 - player;
    int maxEval = std::numeric_limits<int>::min();
    int minEval = std::numeric_limits<int>::max();
    ChessBoard::AIMove currentBestMove;

    if(player == aiType) {

        for(auto &m : moves) {
            // 保存原始状态
            uint64_t oldKey = currentZobristKey;
            int oldQx = board[m.qx][m.qy];
            int oldMx = board[m.mx][m.my];
            int oldAx = board[m.ax][m.ay];

            // 执行移动
            currentZobristKey ^= ZobristKeys[m.qx][m.qy][player]; // 移除旧 Queen
            currentZobristKey ^= ZobristKeys[m.mx][m.my][player]; // 放置新 Queen
            currentZobristKey ^= ZobristKeys[m.ax][m.ay][3];      // 放置 Arrow
        ...
        for(auto &m : moves) {
            ...
            currentZobristKey ^= ZobristKeys[m.qx][m.qy][player]; // 移除旧 Queen
            currentZobristKey ^= ZobristKeys[m.mx][m.my][player]; // 放置新 Queen
            currentZobristKey ^= ZobristKeys[m.ax][m.ay][3];
            board[m.qx][m.qy] = 0;
            board[m.mx][m.my] = player;
            board[m.ax][m.ay] = 3;
    ...
    int finalScore = (player == aiType) ? maxEval : minEval;

    // 确定标志位
    int flag = TT_FLAG_EXACT;
    if (finalScore <= originalAlpha) {
        flag = TT_FLAG_BETA; // 实际值可能小于等于 score (上限)
    } else if (finalScore >= originalBeta) {
        flag = TT_FLAG_ALPHA; // 实际值可能大于等于 score (下限)
    }
...
int ChessBoard::advancedEvaluate(int player) {
    ...
    // 加权组合
    score += 100 * (myMobility - oppMobility);        // 移动性权重最高
    score += 150 * (myTerritory - oppTerritory);      // 领地控制
    ...
    if(getDynamicDepth() == 4) {
        score += 100 * (myMobility - oppMobility);
        score += 150 * (myTerritory - oppTerritory);
    }
    else {
        score += 300 * (myMobility - oppMobility); // 移动性权重大幅提高
        score += 50 * (myTerritory - oppTerritory);
    }
    return score;
}
...
void initializeZobristKeys() {
    ...
    for (int i = 0; i < 10; ++i) {
        for (int j = 0; j < 10; ++j) {
            // piece_type = 0 (Empty) 不需要Key，因为它总是 XOR 0
            for (int k = 1; k <= 3; ++k) {
                ZobristKeys[i][j][k] = rng();
            }
        }
    }
}
